...
Handle* LRUCache<KeyType, ValueType, LockType>::insert(const KeyType& key, uint32_t hash, ValueType* value, size_t charge, void (*deleter)(const KeyType& key, ValueType* value)) {
    ...
    if (it != lruMap.end()) {
        ...
        finshErase(node); // Finish erasing the old node
        usage_ += charge;
    }
    else {
    ...
    while (usage_ > capacity_ && !lruList.empty()) {
        ...
        lruMap.erase(node->key); // Remove the key from the LRU map
        usage_ -= node->charge; // Update the cache usage
        node->deleter(*(node->key), node->value); // Call the deleter function
        delete[] reinterpret_cast<char*>(node);  // Delete the node
    }
...
void LRUCache<KeyType, ValueType, LockType>::unref(LRUNode<KeyType, ValueType>* node) {
    node->refs--;
    if (node->refs == 0 || node->inCache == false) {
        (*node->deleter)(*(node->key), node->value);
        free(node);
    }
